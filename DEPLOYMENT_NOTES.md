...
Before each production image build, the deployment lane removes unused BuildKit cache older than 48 hours. This prevents release cache from filling the dedicated host while leaving tagged release images available for rollback. Override the window only with a positive hour or minute duration in `DOCKER_BUILD_CACHE_RETENTION`.
...
Private Supabase Storage evidence must also be copied daily to a separate S3-compatible account. Copy `deploy/production/backup.env.example` to `/srv/losttofound/config/backup.env`, keep it owned by the deployment user with mode `0600`, and use a bucket-scoped application key. The bucket and every uploaded object must use Compliance Object Lock. Backblaze is configured for 178 days of lock followed by deletion one day after hiding, keeping the combined lifecycle below the 180-day maximum even though its generated read/write keys include an inapplicable Governance-bypass capability. Deployments install `custodyfolio-storage-backup.timer` only after that secret file exists. The readiness endpoint blocks when the key is within 30 days of expiry. Run `npm run verify:storage-backup` inside the one-shot backup container after the first backup and at each restore drill. Supabase database backups do not include Storage objects.
