...
- [x] On August 27, 2026, the operator approved United States-only direct Stripe checkout with `STRIPE_TAX_MODE=not_collecting` and monthly Stripe Tax threshold monitoring. App Store distribution remains worldwide. This is disclosed operator self-review, not professional tax advice or a claim that no future registration can arise.
...
- [x] Hosted Checkout requires a United States service address through Stripe's country allowlist, states that the product is digital and nothing is shipped, and records `us_only` in Checkout and subscription metadata.
...
- [x] TestFlight build 17 is `VALID` and `IN_BETA_TESTING` in External Beta; the public link and TesterBuddy redirect resolve correctly, with 3 of 10 tester seats in use. Reverified read-only on 2026-08-27.
- [x] Build 17 contains the required-reason privacy-manifest correction and is the current release-candidate binary.
